class FinancialAdvisor:
    def __init__(self, balance, transaction_data, goals_data):
        ...
        for i in range(len(transaction_data)):
            if transaction_data[i][0] > 0:
                self.income += transaction_data[i][0]
            else:
                self.spending += abs(transaction_data[i][0])

            if transaction_data[i][1] in ['Groceries', 'Rent', 'Utilities']:
                self.spending_on_needs += abs(transaction_data[i][0])
        ...
        if not len(goals_data) == 0:
            for i in range(len(goals_data)):
                if goals_data[i][0] == 'emergency_fund':
                    self.current_fund += goals_data[i][1]

                self.amount_saved += goals_data[i][1]
                self.total += goals_data[i][2]

        self.goal_progress = round(self.amount_saved / (self.total + 1), 2)
        self.advice = []
    ...
    def emergency_fund(self, current_fund, spending):
